import React from 'react'
import ScrollUp from './ScrollUp.js'
import palms from "./images/palms.jpeg"
import drmixa from "./images/drmixa.webp"
import mrsmixa from "./images/mrsmixa.webp"


const About = () => {
    return (
        <main>
            <div className='about-title-container'>
                <h1>About us</h1>
            </div>
            <div className='about-wrapper'>
                <div className='about-intro-container'>
                    <div className='about-intro-wrapper'>
                        <div className='about-intro-text'>
                            <h2>Our Practice</h2>
                            <p>Mixa Orthopedics has been open as a private orthopedic practice since 2008. Dr. Mixa works with his wife, Martha Mixa, who is his Physician's Assistant. While running his own practice, Dr. Mixa is also on call and seeing patients in the hospital at over 4 different hospitals in the Tampa Bay area. <br /><br /> As said by the American Academy of Orthopedic Surgeons, "Orthopedics is the medical specialty devoted to the diagnosis, treatment, rehabilitation, and prevention of injuries and diseases of your body's musculoskeletal system. This complex system included your bones, joints, ligaments, tendons, muscles, and nerves and allows you to move, work, and be active."
                            </p>
                        </div>
                        <div className='about-intro-picture'>
                            <img src={palms} alt='office'></img>
                        </div>
                    </div>
                </div>
            </div>
                <div className='meet-practicioners'>
                    <h1>Meet our Practicioners</h1>
                </div>
                <div className='practicioner-container'>
                    <div className='about-wrapper'>
                        <div className='practicioner-wrapper'>
                            <div>
                                <img src={drmixa} alt='drmixa'></img>
                            </div>
                            <div>
                                <div className='practicioner-text'>
                                    <h2>Dr. Thomas Mixa, M.D.</h2>
                                    <p>Dr. Mixa is a Johns Hopkins graduate trained by world renowned total joint surgeons, Dr. David Hungerford, Dr. Kenneth Krackow, and Dr. Anthony Hedley. He has performed over 5,000 joint replacements with exceptional results over the past 28 years. </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='about-wrapper'>
                    <div className='drmixa-resume-container'>
                        <div className='drmixa-resume-wrapper'>
                            <div>
                                <div className='drmixa-resume-column'>
                                    <h3>Residency</h3>
                                    <p>Johns Hopkins Hospital, Orthopedics, Baltimore, Maryland</p>
                                    <p>Catholic Medical Center, Orthopedics, New York, New York</p>
                                    <p>Metro Group Hospitals, University of Illinois, Internship General Surgery, Chicago, Illinois</p>
                                </div>
                                <div className='drmixa-resume-column'>
                                    <h3>Education</h3>
                                    <p>Loyola University of Chicago, IL graduated 1979 Bachelor of Science in Biology</p>
                                    <p>The Chicago Medical School, Chicago, IL graduated 1985 Medical Degree (M.D.)</p>
                                    <p>University of Illinois, Chicago, IL graduated 1986 <br></br>General Surgery Internship</p>
                                    <p>Johns Hopkins Hospital, Baltimore, MD graduated 1991 Orthopedic Residency</p>
                                    <p>The Institute for Bone and Joint Disease, Phoenix, AZ graduated 1992Total Joint Fellowship</p>
                                </div>
                            </div>
                            <div>
                                <div className='drmixa-resume-column'>
                                    <h3>Hospital Positions</h3>
                                    <p>Multiple Committee Positions, over 20+ yearsInfectious Disease, Utilization Review, Surgical Boards</p>
                                    <p>Chief of Orthopedics 2004-2005 St. Anthony's Hospital, St. Petersburg, FL</p>
                                    <p>Surgical Governance Committee 2012-2020 St. Anthony's Hospital, St. Petersburg, FL</p>
                                    <p>Section Chief of Orthopedics 2012-2020St. Anthony's Hospital, St. Petersburg, FL</p>
                                </div>
                                <div className='drmixa-resume-column'>
                                    <h3>Professional Positions</h3>
                                    <p>Anatomy Instructor 1977-1979 Loyola University of Chicago</p>
                                    <p>Team Physician for Tampa Bay Devil Rays 1998-2006 St. Petersburg, Florida</p>
                                    <p>Clinical Instructor Family Medicine Residency Program at HCA Florida St. Petersburg Hospital  2010- PresentFamily Physician Residency Program</p>
                                    <p>Clinical Professor NOVA 2009-Present <br /> Physician Assistant Residency Program</p>
                                </div>
                                <div className='drmixa-resume-column'>
                                    <h3>Volunteer Positions</h3>
                                    <p>Faith in Practice Medical Missions to Antigua, Guatemala 2011-2018 <br /> Antigua, Guatemala</p>
                                    <p>St. Petersburg Free Clinic, 2009-2018 <br /> St. Petersburg, Florida </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='practicioner-container'>
                    <div className='about-wrapper'>
                        <div className='practicioner-wrapper'>
                            <div>
                                <img src={mrsmixa} alt='mrsmixa'></img>
                            </div>
                            <div>
                                <div className='practicioner-text'>
                                    <h2>Martha Mixa, P.A.-C</h2>
                                    <p>Martha has been practicing as a Physician's Assistant for over 10 years.  She and Dr. Mixa continue to work day to day at Mixa Orthopedics and their affiliated hospitals to provide their patients with the best care possible.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='about-wrapper'>
                    <div className='mrsmixa-resume-container'>
                        <div className='drmixa-resume-wrapper'>
                            <div>
                                <div className='drmixa-resume-column'>
                                    <h3>Professional Positions</h3>
                                    <p>Surgical/Vascular Physician's Assistant <br /> 1985-1987</p>
                                </div>
                            </div>
                            <div>
                                <div className='drmixa-resume-column'>
                                    <h3>Education</h3>
                                    <p>St. Johns University, Jamacia, New York <br /> Bachelor's of Science in Biology <br /> Physician's Assistant</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            {/* </div> */}
            <ScrollUp />
        </main>
    )
}

export default About